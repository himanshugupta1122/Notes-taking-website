...
export default function SingleNotice({ item }) {
    ...
    return (
        ...
        <div style={{ margin: "40px 45px", padding: "30px", backgroundColor: "white" }} className="drop-shadow-2xl cards-borde">
            <div>
                {edit ? <h1 className='font-bold text-xl overflow-auto'> {item.notice}</h1> :<input className='bg-transperent border-input' onChange={(e) => settask(e.target.value)} value={task}></input>}
